namespace Supercell.Laser.Server.Message
{
    ...
    public class MessageManager
    {
        ...
        {
            string abd = $"Пинг: {GetPingIconByMs(0)} (---ms)\n";
            if (Connection.Ping != 0)
            {
                abd = $"Пинг: {GetPingIconByMs(Connection.Ping)} ({Connection.Ping}ms)\n";
            }
            ...
            {
                LobbyData = $"KSD Brawl Reborn\n<cff9f00>T<cffaa00>G<cffb400>:<cffbf00> <cffca00>@<cffd400>k<cffdf00>s<cfee900>d<cfff400>b<cffff00>r<cffff00>a<cffff19>w<cffff32>l<cffff4c>r<cffff65>e<cffff7f>b<cffff98>o<cffffb2>r<cfefecb>n</c>\n{abd}Онлайн: {Sessions.Count}\nВремя: {DateTime.Now}\nПремиум: {hasPremium}\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n",
                PlayersCount = 0
            };
            Connection.Send(b);
        }
                ...
                switch (cmd[0])
                {
                    case "status":
                        ...
                        response.Entry.Message = $"Server Status:\n" +
                            $"Серверная версия игры: v28.289\n" +
                            $"Билд сервера: v1.2 с 15.01.2024\n" +
                            $"SHA Ресурсы: {Fingerprint.Sha}\n" +
                            $"Среда: Production\n" +
                            $"Серверное время: {DateTime.Now} EEST\n" +
                            $"Онлайн: {Sessions.Count}\n" +
                            $"Использовано ОЗУ: {megabytesUsed} MB\n" +
                            $"Запущен: {formattedTime}\n";
                        Connection.Send(response);
                        break;
                    case "help":
                        response.Entry.Message = $"Доступные команды:\n/help - показывает список команд\n/status - показывает статус сервера\n KSD ID\n/register [ник] [пароль] - зарегистрировать аккаунт\n/login [ник] [пароль] - зайти в аккаунт"; // /usecode [code] - use bonus code
                        Connection.Send(response);
                        break;
                    // ACCOUNT SYSTEM HERE
                    case "register":
                        if (cmd.Length != 3)
                        {
                            response.Entry.Message = $"Использование: /register [ник] [пароль]";
                            Connection.Send(response);
                            return;
                        }
                        ...
                        {
                            response.Entry.Message = $"Данный ник уже занят!";
                            Connection.Send(response);
                            return;
                        }
                        ...
                        Notification brlyn = new()
                        {
                            Id = 89,
                            DonationCount = 30,
                            MessageEntry = "KSD ID - <c6>Спасибо за привязку!</c>"
                        };
                    ...
                    case "login":
                        if (cmd.Length != 3)
                        {
                            response.Entry.Message = $"Использование: /login [ник] [пароль]";
                            Connection.Send(response);
                            return;
                        }
                        ...
                        {
                            ErrorCode = 8,
                            Message = "Вход в аккаунт успешен."
                        });
                        break;
                    default:
                        response.Entry.Message = $"Неизвестная команда /{cmd[0]}!";
                        Connection.Send(response);
                        break;
                }
        ...
        private void AskForBattleEndReceived(AskForBattleEndMessage message)
        {
            ...
            if (DateTime.UtcNow > HomeMode.Home.PremiumEndTime && HomeMode.Avatar.PremiumLevel > 1)
            {
                ...
                HomeMode.Home.NotificationFactory.Add(new Notification
                {
                    Id = 81,
                    //TimePassed = 
                    MessageEntry = $"Привет!\n У тебя закончился твой vip статус\nпродлить его можно у администратора, тг: @well0ff"
                });
            }
            ...
            if (isPvP)
            {
                ...
                if (slot == 9)
                {
                    if (HomeMode.Home.PowerPlayGamesPlayed < 3)
                    {
                        ...
                        {
                            if (DateTime.UtcNow.Subtract(HomeMode.Avatar.BattleStartTime).TotalSeconds <= 0)
                            {
                ...
                else if (location.GameMode == "BountyHunter" || location.GameMode == "CoinRush" || location.GameMode == "AttackDefend" || location.GameMode == "LaserBall" || location.GameMode == "RoboWars" || location.GameMode == "KingOfHill")
                {
                    ...
                    if (message.BattleResult == 0) // Win
                    {
                        ...
                        if (location.GameMode == "CoinRush")
                        {
                            ...
                            {
                                underdogTrophiesResult += (int)Math.Round((double)Trophies[message.BattleResult] / 2);
                ...
                else if (location.GameMode == "BattleRoyale")
                {
                    ...
                    {
                        Trophies = new[] { 20, 16, 14, 12, 6, 4, 4, 0, -1, -2 };
                    }
                    else if (brawlerTrophies >= 100 && brawlerTrophies <= 199)
                    {
                        Trophies = new[] { 20, 16, 14, 12, 6, 2, 0, -1, -2, -2 };
                    }
                    ...
                    TokensRewards = new[] { 60, 30, 24, 21, 15, 12, 8, 6, 4, 2, 0 };
        ...
        private void GetLeaderboardReceived(GetLeaderboardMessage message)
        {
            ...
            else if (message.LeaderboardType == 0)
            {
                ...
                aaaa = aaaa.Take(Math.Min(aaaa.Count, 150)).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
        ...
        private void LoginReceived(AuthenticationMessage message)
        {
            ...
            if (account.Avatar.Banned)
            {
                ...
                if (account.Avatar.Banned)
                {
                    ...
                    {
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: намеренное эксплуатация ошибок!";
                    }
                    ...
                    {
                        ban_message = "Ваш аккаунт заблокирован!\nПричина: Обман администрации/игроков!";
                    }
        ...
        private void ClientHelloReceived(ClientHelloMessage message)
        {
            ...
            Connection.Messaging.serversc = (byte)r.Next(1, 1000);
